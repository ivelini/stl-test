...
### Конкурентность (вне стандартного контура, способ исполнения — обсудить)

**Тест 20** — 200 конкурентных подтверждений на слот вместимостью 10 → подтверждено ровно 10 (мера характеристики 1)
Ожидание: confirmed == 10, остальные отказы
Имя: `test_200_concurrent_confirms_yield_exactly_capacity`

## Прогоны

_Заполняется при реализации: красный прогон (до файлов в app/) и зелёный (после)._
